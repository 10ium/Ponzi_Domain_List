...
jobs:
  extract_domains:
    ...
    steps:
      ...
      - name: Run the script # اجرای اسکریپت پایتون و ذخیره خروجی
        run: python extract_domains.py > output.txt
      - name: show output.txt content
        run: cat output.txt
      - name: Commit and push results # اضافه کردن برای کامیت و فش خروجی (اختیاری)
        ...
        run: |
          ...
          git add output.txt # فایل output.txt را اضافه می کند
